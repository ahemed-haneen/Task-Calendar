...
const CalendarWeekBar: FC<{ date: Date, weekdays?: Array<string>, tasks: Array<TaskProps> }> = ({ date, tasks }) => {
    ...
    return <div className="flex flex-row gap-1">
        ...
        <div className='grid bg-stone-100 grid-auto-rows grow grid-cols-7 grid-flow-dense'>
            {tasks.map(task => {
                const IS_DATE_BEFORE = !isDateInRange(date, task.startDate, 'week', 'before');
                const IS_DATE_AFTER = !isDateInRange(date, task.endDate, 'week', 'after');
                ...
                return <div className={`text-left px-2 py-2 text-white font-semibold text-xs m-0.5` + (IS_DATE_BEFORE ? '' : ' rounded-l-xl') + (IS_DATE_AFTER ? '': ' rounded-r-xl')} 
                    style={{ 
